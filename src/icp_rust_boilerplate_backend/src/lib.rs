...
// The type of the counter that is used to generate unique ids for messages.
type IdCell = Cell<u64, Memory>;

// The message struct that is stored in the stable storage.
...
// Thread local storage for the memory manager, id counter and the storage.
// This is a simple way to store the state of the canister.
// In a real world application, we would want to use a database or a more sophisticated storage mechanism.
thread_local! {
...
// Message payload that is used to create a new message.
...
// Error type that can be returned from the canister.
